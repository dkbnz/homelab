...
- **RAM**: 16 GB
- **Storage**:
  - `sda` 64 GB SanDisk SSD - boot, `local` and `local-lvm`
  - `sdb` 931 GB Samsung T7 - data disk, passed through to the Docker LXC (CT 102)
...
2. Recreate each guest with the matching community helper script, then apply the
   ...
   - CT 102: Docker script, pass through `sdb` (`pct set 102 -dev0 /dev/sdb`), then
     `docker compose -f guests/docker/watchtower.compose.yml up -d`.
